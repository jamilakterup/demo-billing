...
namespace App\Http\Livewire\InactiveLead;

use App\Models\LeadCollection;
use Livewire\Component;

class InactiveTable extends Component
{
    protected $listeners = [
    ...
    {
        $leadCollections = LeadCollection::where('status', 'inactive')
            ->where(function ($q) {
                if ($this->searchField) {
    ...
    {
        $this->leadCollections = LeadCollection::where('status', 'inactive')
            ->where(function ($q) {
                if ($this->searchField) {
                    $q->where('name', 'like', '%' . $this->searchField . '%')
                        ->orWhere('phone', 'like', '%' . $this->searchField . '%')
        ...
        return view('livewire.inactive-lead.inactive-table');
    }
}
