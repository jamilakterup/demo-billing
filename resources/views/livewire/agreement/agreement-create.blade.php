<form wire:submit.prevent="agreementStore" class="p-2">
    <div class="border bg-light rounded p-4">
        <div class="form-row">
            <div class="form-group col-md-4">
                <label for="inputEmail4">Customer</label>

                <select wire:model="state.customer_id" style="width: 350px" id="customer_id" class="tom-select">
                    <option value="">--Please Select--</option>
                    @foreach ($customers as $key=>$value)
                    <option value="{{$key}}" @if(isset($state['customer_id']) && $state['customer_id']==$key) selected
                        @endif>{{$value}}</option>
                    @endforeach
                </select>

                @if($errors->has('customer_id'))
                <div class="text-danger text-sm"><small>{{ $errors->first('customer_id')
                        }}</small>
                </div>
                @endif
            </div>
            {{-- {{dd($state)}} --}}
            <div class="form-group col-md-4">
                <label for="inputEmail4">Agreement Number <span class="text-danger">*</span></label>
                <input type="text" wire:model.defer="state.number" class="form-control" id=""
                    aria-describedby="emailHelp" placeholder="Enter agreement number" disabled>
            ...
            <div class="form-group col-md-4">
                ...
                <select class="tom-select form-control @error('agreement_type_id') is-invalid @enderror"
                    wire:model.defer="state.agreement_type_id" wire:change="agreementType($event.target.value)">
                    <option value="">--Please Select--</option>
                    @foreach($agreementTypes as $agreementType)
                    <option value="{{$agreementType['id']}}" @if(isset($state['agreement_type_id']) &&
                        $state['agreement_type_id']==$agreementType['id']) selected @endif>
                        {{$agreementType['agreement_type_name']}}</option>
                    @endforeach
                </select>


                @if($errors->has('agreement_type_id'))
                <div class="text-danger text-sm"><small>{{ $errors->first('agreement_type_id') }}</small></div>
                @endif
            </div>
        ...
        <div class="form-row">
            <div class="form-group col-md-4">
                <label for="inputEmail4">Agreement Date <span class="text-danger">*</span></label>
                <input type="date" wire:model.defer="state.agreement_date" class="datepicker form-control" id=""
                    aria-describedby="emailHelp" placeholder="agreement Date">
    ...
    <div class="modal-footer justify-content-center">
        ...
        <button wire:click.prevent="agreementStore" wire:loading.attr="disabled" class="btn btn-primary">
            <div wire:loading.remove wire:target="agreementStore">
                <span><i class="fas fa-save mr-1"></i> Submit</span>
            </div>

            <div wire:loading="agreementStore" wire:target="agreementStore">
                <div class="d-flex gap-2 align-items-center">
                    <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                    <span class="ml-1">Loading...</span>
                </div>
