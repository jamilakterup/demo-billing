                        ...
                        <div class="card">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            ...
                                            @if ($invoice->cancellation == 0)
                                                <form action="{{ route('invoice.update', $invoice->id) }}" method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit" class="dropdown-item">Inactive invoice</button>
                                                </form>
                                            @elseif($invoice->cancellation == 1)
                                                <form action="{{ route('invoice.update', $invoice->id) }}" method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit" class="dropdown-item">Active invoice</button>
                                                </form>
                                            @endif
                            ...
                            <div class="card-body">
                                ...
                                <div id="pdf-viewer-container" class="pdfobject" class="border rounded">
                                    <iframe src="{{ asset('/pdf/' . $invoice->file) }}"
                                        style="overflow: auto; width: 100%; min-height: 500px;">
