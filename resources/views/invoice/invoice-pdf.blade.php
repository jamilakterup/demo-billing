...
<body>
    ...
    <table style="width:100%; border:none">
      <tr>
        <td style="border:none;text-align:left; padding:0">
          Ref-{{$invoice->invoice_type->invoice_type_short_name}}{{date('y')}}{{date('m')}}{{$invoice->number}}</td>
    ...
    <table id="details">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="font-family:siliguri">{{$invoice_detail->product->name}}</td>
          ...
          <td style="text-align:right">{{number_format((float)$invoice_detail->price*$invoice_detail->quantity, 2, '.',
            ',')}}
          </td>
        </tr>
        @php
      ...
      <tfoot>
        ...
        @if($invoice->vat>0)
        <tr>
          <td colspan="6" style="text-align:center">VAT {{number_format((float)(($invoice->vat*100)/$invoice->sub_total), 2, '.', ',')}} %</td>
        ...
        @if($invoice->tax>0)
        <tr>
          <td colspan="6" style="text-align:center">TAX {{number_format((float)(($invoice->tax*100)/$invoice->sub_total), 2, '.', ',')}} %</td>
  ...
  ?>
    ...
    <ul>
      <li>Payment should be made by an account payee cheque in favor of "rajIT Solutions Ltd." A/C No. 0044-0210006856
