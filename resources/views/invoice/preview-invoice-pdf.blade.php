...
<body>
  ...
  <div class="container">
    ...
    <table style="width:100%; border:none">
      <tr>
        <td style="border:none;text-align:left; padding:0">
          Ref-{{$invoiceType->invoice_type_short_name}}{{date('y')}}{{date('m')}}{{$invoice['number']}}</td>
    ...
    <table id="details">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td style="text-align:right">{{number_format((float)$invoice_detail->price*$invoice_detail->quantity, 2, '.',
            ',')}}</td>
          @if ($invoice_detail->imagee)
          <td class="text-center">
            <img style="height:45px; width:45px" src="{{ asset('storage/' . $invoice_detail->imagee) }}"
              alt="product image">
          </td>
          @endif
        </tr>
      ...
      <tfoot>
        ...
        @if($cartTax>0)
        <tr>
          <td colspan="5" style="text-align:center">IT ({{number_format((float)($cartTax*100)/$total, 2, '.', '')}} %)
          </td>
  ...
  ?>
    ...
    <h4>N.B: {{$invoice['vat_text_visibility']=="VAT & TAX. Paid by"?'VAT & TAX. Paid by
      '.$customer->company_name:$invoice['vat_text_visibility']}}</h4>
    @endif
